 ...
 * File:  MRF24J40_lib.h
 *
 * Wireless communications library definitions for MRF24J40 RF module.
 *
 * Acknowledgements:
...
#include "gendef.h"      // MJB's common def's for embedded C applications
#include "SPI_drv.h"     // SPI driver functions to suit the host MCU
#include "low_level.h"   // I/O pin manipulation macros to suit the host MCU

//#define USE_MRF_LEGACY_CODE

// Select SPI channel to use (SPI1 or SPI2) here...
#define SPI_Initialize(m)     SPI1_Initialize((uint8)(m))
#define SPI_Exchange8bit(b)   SPI1_Exchange8bit((uint8)(b))
// SPI clock mode for MFP24J40 is mode 0
#define SPI_MODE0             (0)
...
// max data length = (127 aMaxPHYPacketSize - 2 Frame control - 1 sequence number - 2 panid 
//                    - 2 shortAddr Destination - 2 shortAddr Source - 2 FCS)
//
